...
try:
    ...
    if api_key:
        st.sidebar.success("Gemini API key loaded from key.txt!")
    else:
        st.sidebar.warning("key.txt file is empty. Please add your API key to it.")
except FileNotFoundError:
    st.session_state.gemini_api_key = ""
    st.sidebar.error("key.txt not found. Please create this file and paste your Gemini API key inside.")

if st.sidebar.button("Get AI Suggestions"):
    if not st.session_state.gemini_api_key:
        st.sidebar.error("Cannot get AI suggestions without a valid API key.")
    else:
        st.session_state.ai_suggestions = {}
